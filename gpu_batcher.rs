...
/// A request with its channel for streaming
...
/// Memory-aware dynamic GPU batching
    ...
    pub async fn start(self: Arc<Self>) {
        ...
        tokio::spawn(async move {
            loop {
                ...
                // Process each request independently, assigning GPU shard
                for req in batch_requests {
                    let request_tokens = req.messages.iter().map(|m| m.content.split_whitespace().count()).sum();
                    if let Some(shard) = gpu_mgr.select_shard(&req.model_name, request_tokens).await {
                        // Spawn hybrid streaming task per request
                        ...
                        tokio::spawn(async move {
                            let mut token_stream = schedule_inference_stream(&model_name, req.backend, &messages, provider.as_deref()).await;

                            while let Some(token) = token_stream.recv().await {
                                let _ = sender.send(token).await;
                            }

                            gpu_mgr.release_load(&model_name, shard.gpu_id, request_tokens).await;
                        });
                    } else {
                        eprintln!("No available GPU shard for model {}", req.model_name);
                    }
                }
            }
        });
